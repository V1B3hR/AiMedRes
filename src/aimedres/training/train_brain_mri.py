...
class BrainMRITrainingPipeline:
    ...
    def load_dataset(self, dataset_choice: str = "original") -> Tuple[List[str], List[int]]:
        ...
        try:
            ...
            if dataset_choice == "original":
                ...
                for path in image_paths:
                    ...
                    if any(word in path.lower() for word in ["mild", "moderate", "severe"]):
                        ...
                        if any(
                            word in " ".join(parent_dirs)
                            for word in ["alzheimer", "dementia", "ad"]
                        ):
                            labels.append(1)
                        else:
                            labels.append(0)
            ...
            logger.info(
                f"Label distribution: Class 0: {labels.count(0)}, Class 1: {labels.count(1)}"
            )
    ...
    def train_model(
        ...
        use_3d: bool = False,
        mlflow_experiment: str = "brain_mri_classification",
        dataset_choice: str = "original",
    ) -> Dict[str, Any]:
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(
                    model.state_dict(), self.output_dir / "models" / "best_brain_mri_model.pth"
                )
...
def main():
    ...
    parser.add_argument(
        "--epochs", type=int, default=20, help="Number of epochs for training (default: 20)"
    )
    parser.add_argument(
        "--batch-size", type=int, default=32, help="Batch size for training (default: 32)"
    )
    parser.add_argument(
        "--use-3d", action="store_true", help="Use 3D CNN instead of 2D CNN for volumetric data"
    )
    parser.add_argument(
        "--mlflow-experiment",
        type=str,
