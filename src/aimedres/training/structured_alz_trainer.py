...
class EarlyStoppingMLP:
    ...
    def fit(self, X_train, y_train, X_val, y_val, epochs: int = 80, batch_size: int = 64):
        ...
        for epoch in range(1, epochs + 1):
            ...
            logger.debug(
                f"Epoch {epoch}: val_acc={val_acc:.4f}, val_f1={val_f1:.4f}, "
            ...
            if self.early_stop_state.epochs_no_improve >= self.patience:
                logger.info(
                    f"Early stopping at epoch {epoch} (best: {self.early_stop_state.best_epoch})"
                )
                break
...
class StructuredAlzTrainer:
    ...
    def preprocess(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series, ColumnTransformer]:
        ...
        high_missing = missing_pct[
            missing_pct > self.config.get("preprocessing", {}).get("missing_threshold", 10)
        ]
        ...
        logger.info(
            f"Feature types: {len(numeric_cols)} numeric, {len(categorical_cols)} categorical"
        )
            ...
            numeric_pipeline = Pipeline(
                [
                    (
                        "impute",
                        SimpleImputer(
                            strategy=self.config.get("preprocessing", {}).get(
            ...
            categorical_pipeline = Pipeline(
                [
                    ...
                    (
                        "onehot",
                        OneHotEncoder(
                            handle_unknown=self.config.get("preprocessing", {}).get(
        ...
        self.feature_names = {"numeric": numeric_cols, "categorical": categorical_cols}
        ...
        for model_name in self.config.get("models", []):
            ...
            if model_name == "logreg":
                ...
                models["mlp"] = MLPClassifier(
                    max_iter=1, warm_start=True, **params  # We'll use partial_fit
                )
            else:
    ...
    def train_model(self, model_name: str, X_train, y_train, X_val, y_val) -> Dict[str, Any]:
        ...
        if model_name == "mlp":
            ...
            epoch_metrics = mlp_wrapper.fit(
                X_train, y_train, X_val, y_val, epochs=self.epochs, batch_size=self.batch_size
            )
    ...
    def save_artifacts(
        self, best_model_name: str, final_model_name: str, run_metrics: Dict[str, Any]
    ) -> None:
        """
        ...
        # Save final model
        if (
            artifacts_config.get("save_final_model", True)
            and final_model_name in self.trained_models
        ):
    ...
    def run_full_training(self, data_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
        ...
        X_train, X_val, y_train, y_val = train_test_split(
            X_transformed, y, test_size=self.validation_split, random_state=self.seed, stratify=y
        )
