...
class KnowledgeGraph:
    ...
    def add_edge(
        self, source_id: str, target_id: str, relationship: str, attributes: Dict[str, Any] = None
    ) -> None:
...
def populate_sample_knowledge_graph(kg: KnowledgeGraph):
    ...
    kg.add_node(
        "T0003",
        "treatment",
        {"name": "Lifestyle changes", "details": "Diet, exercise, weight loss"},
    )
...
class MockLanguageModel:
    ...
    def generate_summary(self, context: str, query: str) -> str:
        ...
        return (
            f"Based on the available information regarding '{query}', here is a summary: "
...
def handle_task():
    ...
    if not isinstance(task, str) or len(task) > Config.MAX_TASK_LENGTH:
        return (
            jsonify(
                {
                    "error": f"Invalid 'task'. Must be a string under {Config.MAX_TASK_LENGTH} characters."
                }
            ),
            400,
        )
