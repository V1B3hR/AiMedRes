    ...
    def is_rate_limited(self, key: str, limit: int, window: int) -> bool:
        ...
        try:
            if self.redis:
                ...
                # Clean old requests
                self.local_cache[key] = [
                    req_time for req_time in self.local_cache[key] if req_time > now - window
                ]
    ...
    def _register_routes(self):
        ...
        def health_check():
            ...
            status = {
                ...
                "version": "1.0.0",
                "safety": (
                    self.safety_monitor.get_status()
        ...
        def predict():
            ...
            try:
                ...
                self.safety_monitor.record_operation(
                    operation="prediction", duration=0.1, success=True  # Placeholder
                )
                ...
                self.safety_monitor.record_operation(
                    operation="prediction", duration=0.1, success=False, error=str(e)
                )
        ...
        if self.config.api.ssl_enabled and ssl_context is None:
            if self.config.api.ssl_cert_path and self.config.api.ssl_key_path:
                ...
                ssl_context.load_cert_chain(
                    self.config.api.ssl_cert_path, self.config.api.ssl_key_path
                )
