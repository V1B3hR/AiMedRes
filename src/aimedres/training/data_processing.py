...
def preprocess_alzheimer_data(
    df: pd.DataFrame, target_col: str = "diagnosis", optimization_level: str = "standard"
...
def add_medical_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "age" in df.columns:
        df["age_group"] = pd.cut(
            df["age"], bins=[0, 65, 75, 85, 100], labels=[0, 1, 2, 3]
        )  # Use numeric labels
    ...
    if "apoe_genotype" in df.columns:
        df["apoe_risk_score"] = (
            df["apoe_genotype"]
            .map({"E2/E2": 0, "E2/E3": 0, "E2/E4": 1, "E3/E3": 1, "E3/E4": 2, "E4/E4": 3})
            .fillna(1)
...
def normalize_features(X: pd.DataFrame, method: str = "standard") -> Tuple[pd.DataFrame, object]:
    ...
    if len(numerical_columns) > 0:
        ...
        logger.info(
            f"Normalized {len(numerical_columns)} numerical features using {method} scaling"
        )
...
def select_features(
    X: pd.DataFrame, y: pd.Series, k: int = "all", method: str = "f_classif"
