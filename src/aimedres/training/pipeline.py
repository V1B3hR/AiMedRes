...
class TrainingPipeline:
    ...
    def train_network(
        ...
        try:
            ...
            for epoch in range(epochs):
                ...
                logger.info(
                    f"Epoch {epoch + 1}/{epochs} complete: {epoch_samples} samples in {epoch_time:.2f}s"
                )
            ...
            final_accuracy = (
                self._calculate_accuracy(network, validation_data, validation_labels)
            ...
            logger.info(
                f"Training completed successfully: {epochs_completed} epochs, {total_samples} samples in {training_time:.2f}s"
            )
            return result
    ...
    def train_agent(
        self, agent: DuetMindAgent, training_scenarios: List[Dict[str, Any]], epochs: int = 20
    ) -> TrainingResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Starting agent training: {len(training_scenarios)} scenarios, {epochs} epochs"
            )
            ...
            logger.info(
                f"Agent training completed: {epochs_completed} epochs, {total_interactions} interactions"
            )
            return result
