...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("aimedres.log")],
)
...
class AiMedResApplication:
    ...
    def run_training_mode(self):
        ...
        try:
            ...
            for epoch in range(
                min(10, self.config.neural_network.epochs)
            ):  # Limited epochs for demo
                ...
                logger.info(
                    f"Epoch {epoch + 1} complete: {len(batch_results)} samples in {epoch_time:.2f}s"
                )
        ...
        try:
            ...
            for i in range(3):
                agent = self.create_agent(
                    name=f"SimAgent-{i+1}", input_size=32, hidden_layers=[64, 32, 16], output_size=2
                )
                agents.append(agent)
            ...
            for step in range(20):  # 20 simulation steps
                ...
                simulation_results.append(
                    {"step": step + 1, "step_time": step_time, "agents": step_results}
                )

                logger.info(
                    f"Simulation step {step + 1} complete: {len(step_results)} agents in {step_time:.3f}s"
                )
            ...
            try:
                ...
                if command == "quit" or command == "exit":
                    ...
                    for agent_id, agent in self.agents.items():
                        status = agent.get_status()
                        print(
                            f"  {status['name']}: {status['state']} (energy: {status['biological_state']['energy']:.1f})"
                        )
                    ...
                    for net_id, network in self.neural_networks.items():
                        ...
                        print(
                            f"  {net_id}: {health['network_state']} ({health['active_nodes']} active nodes)"
                        )
                elif command == "status":
    ...
    def print_system_status(self):
        ...
        if self.safety_monitor:
            ...
            print(
                f"Operations: {status['metrics']['total_operations']} (errors: {status['metrics']['error_count']})"
            )
        else:
...
Examples:
  ...
  python -m aimedres                       # Interactive mode (default)
    ...
    parser.add_argument("--config", type=Path, help="Path to configuration file (JSON/YAML)")
    parser.add_argument(
