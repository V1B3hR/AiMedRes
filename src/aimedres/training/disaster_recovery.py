...
class DisasterRecoverySystem:
    ...
    def run_dr_drill(
        self, disaster_type: DisasterType, services: List[str], simulate_data_loss: bool = False
    ) -> DrillResult:
        """
        ...
        try:
            ...
            recommendations = self._generate_recommendations(
                disaster_type, rpo_seconds, rto_seconds, data_loss_detected, failed_services
            )

            # Determine drill success
            drill_successful = (
                rpo_seconds <= self.rpo_config.target_seconds
                and rto_seconds <= self.rto_config.target_seconds
                and not data_loss_detected
                and len(failed_services) == 0
            ...
            recovery_status = (
                ...
                if drill_successful
                else RecoveryStatus.PARTIAL if len(restored_services) > 0 else RecoveryStatus.FAILED
            )

            issues = []
            if rpo_seconds > self.rpo_config.target_seconds:
                issues.append(
                    f"RPO exceeded target: {rpo_seconds:.1f}s > {self.rpo_config.target_seconds}s"
                )
            if rto_seconds > self.rto_config.target_seconds:
                issues.append(
                    f"RTO exceeded target: {rto_seconds:.1f}s > {self.rto_config.target_seconds}s"
                )
            ...
            logger.info(
                f"DR drill {drill_id} completed: {'SUCCESS' if drill_successful else 'ISSUES FOUND'}"
            )
    ...
    def _generate_recommendations(
        ...
        if data_loss_detected:
            recommendations.append("Review backup strategies to minimize data loss")
        ...
        if disaster_type == DisasterType.RANSOMWARE_ATTACK:
            recommendations.append("Enhance security measures to prevent ransomware attacks")
    ...
    def get_rpo_rto_metrics(self) -> Dict[str, Any]:
        ...
        if not self.drill_history:
            return {"total_drills": 0, "message": "No drill history available"}
        ...
        return {
            ...
            "rpo_metrics": {
                ...
                "target_met_count": sum(
                    1 for rpo in rpo_values if rpo <= self.rpo_config.target_seconds
                ),
                "target_met_percent": (
                    sum(1 for rpo in rpo_values if rpo <= self.rpo_config.target_seconds)
                    / len(rpo_values)
                )
                * 100,
            },
            "rto_metrics": {
                ...
                "target_met_count": sum(
                    1 for rto in rto_values if rto <= self.rto_config.target_seconds
                ),
                "target_met_percent": (
                    sum(1 for rto in rto_values if rto <= self.rto_config.target_seconds)
                    / len(rto_values)
                )
                * 100,
            },
            ...
            "last_drill_time": (
                self.drill_history[-1].end_time.isoformat() if self.drill_history else None
            ),
        }
    ...
    def run_comprehensive_drill_suite(self) -> Dict[str, Any]:
        ...
        for disaster_type in DisasterType:
            result = self.run_dr_drill(
                ...
                simulate_data_loss=(
                    disaster_type
                    in [DisasterType.DATABASE_CORRUPTION, DisasterType.RANSOMWARE_ATTACK]
                ),
            )
        ...
        logger.info(
            f"Comprehensive DR drill suite completed: {successful_count}/{len(drill_results)} successful"
        )

        return summary


def create_dr_system(
    ...
    return DisasterRecoverySystem(
        rpo_config=rpo_config, rto_config=rto_config, results_dir=results_dir
    )
