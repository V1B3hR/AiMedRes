...
class AutoMLOptimizer:
    ...
    def optimize(
        ...
        logger.info(
            f"AutoML optimization completed. Best {self.objective_metric}: {self.study.best_value:.4f}"
        )
    ...
    def _objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            if self.X_val is not None and self.y_val is not None:
                ...
                y_pred = (
                    model.predict_proba(self.X_val)[:, 1]
                ...
                scores = cross_val_score(
                    model, self.X_train, self.y_train, cv=cv, scoring=self.objective_metric
                )
                score = scores.mean()
...
def run_automl_pipeline(
    ...
    return {"optimizer": optimizer, "results": results}
