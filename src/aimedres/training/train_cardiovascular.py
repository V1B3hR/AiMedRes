...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("cardiovascular_training.log"), logging.StreamHandler()],
)
...
class CardiovascularTrainingPipeline:
    ...
    def __init__(self, output_dir: str = "cardiovascular_outputs"):
        ...
        logger.info(
            f"Cardiovascular training pipeline initialized. Output directory: {self.output_dir}"
        )
    ...
    def load_data(self, data_path: str = None, dataset_choice: str = "colewelkins") -> pd.DataFrame:
        ...
        if data_path and os.path.exists(data_path):
            ...
            try:
                ...
                logger.info(
                    f"Successfully loaded {len(df)} rows and {len(df.columns)} columns from local file"
                )
                return df
        ...
        try:
            ...
            if "KAGGLE_USERNAME" not in os.environ or "KAGGLE_KEY" not in os.environ:
                logger.warning(
                    "Kaggle credentials not found. Creating sample data for demonstration."
                )
                return self._create_sample_data()
            ...
            if dataset_choice == "colewelkins":
                ...
                logger.warning(
                    f"Unknown dataset choice: {dataset_choice}. Using colewelkins dataset."
                )
                path = kagglehub.dataset_download("colewelkins/cardiovascular-disease")
    ...
    def _create_sample_data(self) -> pd.DataFrame:
        ...
        for i in range(n_samples):
            ...
            if data["gender"][i] == "Male":
                prob += 0.1
            if data["ap_hi"][i] > 140:
                prob += 0.4  # high systolic BP
            if data["ap_lo"][i] > 90:
                prob += 0.3  # high diastolic BP
            if data["cholesterol"][i] == 3:
                prob += 0.3  # high cholesterol
            if data["smoke"][i] == 1:
                prob += 0.2
            if data["alco"][i] == 1:
                prob += 0.1
            if data["active"][i] == 0:
                prob += 0.15  # sedentary lifestyle
            if data["chest_pain"][i] == "Yes":
                prob += 0.3
            if data["shortness_of_breath"][i] == "Yes":
                prob += 0.2
            if data["fatigue"][i] == "Yes":
                prob += 0.1
        ...
        logger.info(
            f"Created sample dataset with {len(df)} rows, cardiovascular disease prevalence: {df['cardio'].mean():.2%}"
        )
    ...
    def train_classical_models(self, n_folds: int = 5) -> Dict[str, Dict[str, float]]:
        ...
        for name, model in models.items():
            ...
            logger.info(
                f"{name} - Accuracy: {results[name]['accuracy']:.4f} (+/- {results[name]['accuracy_std']*2:.4f})"
            )
    ...
    def train_neural_network(self, epochs: int = 100) -> Dict[str, float]:
        ...
        X_tensor = torch.FloatTensor(
            self.X_processed.toarray() if hasattr(self.X_processed, "toarray") else self.X_processed
        )
        ...
        for epoch in range(epochs):
            ...
            if val_loss < best_val_loss:
                ...
                # Save best model
                torch.save(
                    model.state_dict(), self.output_dir / "models" / "best_cardiovascular_nn.pth"
                )
    ...
    def run_full_pipeline(
        ...
        epochs: int = 100,
        n_folds: int = 5,
        dataset_choice: str = "colewelkins",
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Cardiovascular Disease Risk Classification Training Pipeline"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        "--folds", type=int, default=5, help="Number of folds for cross-validation (default: 5)"
    )
