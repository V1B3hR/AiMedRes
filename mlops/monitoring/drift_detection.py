...
class ImagingDriftDetector:
    ...
    def detect_drift(self, new_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        results['summary_metrics'] = {
            ...
            'features_with_drift': sum(1 for fa in results['feature_analysis'].values() 
                                     if fa.get('drift_detected', False)),
    ...
    def _test_feature_drift(
        ...
        if baseline_std > 0:
            ...
            if q25_shift > 1.5 or q75_shift > 1.5:  # Lower threshold for quantiles
                result['drift_detected'] = True
                result['alerts'].append(f"Distribution shift detected in quantiles")
    ...
    def get_drift_summary(self, days: int = 7) -> Dict[str, Any]:
        ...
        summary['feature_drift_frequency'] = dict(sorted(feature_drift_counts.items(), key=lambda x: x[1], reverse=True))
