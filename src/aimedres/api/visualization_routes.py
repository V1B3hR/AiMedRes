...
def create_brain_overlay():
    ...
    try:
        ...
        return jsonify({"success": True, "data": overlay_data}), 200
...
def create_disease_map():
    ...
    try:
        ...
        pathology_data = brain_engine.map_disease_pathology(
            patient_id=patient_id, disease_type=disease_type, severity_map=severity_map
        )

        return jsonify({"success": True, "data": pathology_data}), 200
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "affected_regions": {
                            k.value: v for k, v in snapshot.affected_regions.items()
                        },
...
def visualize_temporal_progression():
    ...
    try:
        ...
        viz_data = brain_engine.visualize_temporal_progression(
            patient_id=patient_id, snapshots=snapshot_ids, time_scale=time_scale
        )

        return jsonify({"success": True, "data": viz_data}), 200
...
def simulate_treatment():
    ...
    try:
        ...
        if not patient_id or not baseline_snapshot_id or not treatment_type_str:
            return (
                jsonify(
                    {"error": "patient_id, baseline_snapshot_id, and treatment_type are required"}
                ),
                400,
            )
...
def compare_treatments():
    ...
    try:
        ...
        comparison = brain_engine.compare_treatment_scenarios(
            patient_id=patient_id, simulation_ids=simulation_ids
        )
...
@require_auth
def get_dicom_explainability(series_id):
    """
        ...
        jsonify(
            {
                ...
                "components": {"brain_engine": "operational", "dicom_viewer": "operational"},
            }
        ),
        200,
    )
