...
class AuditLogExporter:
    ...
    def export_audit_logs(
        ...
        audit_records = self._collect_audit_records(
            filters, include_model_inferences, include_user_actions
        )
    ...
    def generate_compliance_report(
        self, patient_id: str = None, start_date: str = None, end_date: str = None
    ) -> Dict[str, Any]:
        """
        ...
        total_records = len(records)
        user_actions = sum(
            1 for r in records if r["event_type"] in ["access", "modification", "approval"]
        )
        ...
        report = {
            ...
            "period": {"start_date": start_date or "N/A", "end_date": end_date or "N/A"},
            "summary": {
    ...
    def export_model_inference_logs(
        self, model_version: str = None, start_date: str = None, end_date: str = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        all_records = self._collect_audit_records(
            filters, include_model_inferences=True, include_user_actions=False
        )

        # Filter by model version if specified
        if model_version:
            all_records = [
                r for r in all_records if r.get("details", {}).get("model_version") == model_version
            ]
