...
class InputValidator:
    ...
    def sanitize_string(self, value: str, max_length: int = 1000) -> str:
        ...
        sanitized = quote(sanitized, safe='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_.~ ')
        ...
        for field, (min_val, max_val) in medical_ranges.items():
            if field in data:
                ...
                if not (min_val <= value <= max_val):
                    errors.append(f"Invalid range for {field}: must be between {min_val} and {max_val}")
        
        return len(errors) == 0, errors
    
    def validate_json_request(self, json_data: Any, required_fields: List[str] = None) -> Tuple[bool, List[str]]:
        """
