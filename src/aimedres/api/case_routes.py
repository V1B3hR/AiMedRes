...
def approve_case(case_id: str):
    ...
    try:
        ...
        if action not in ['approve', 'reject', 'request_review']:
            return jsonify({'error': 'Invalid action', 'valid_actions': ['approve', 'reject', 'request_review']}), 400
        ...
        updated_case = case_manager.approve_case(
            case_id, user_id, action, rationale, notes
        )
        ...
        return jsonify({
            ...
            'action': action,
            'approved_by': user_id,
            'approved_at': updated_case.get('approved_at') or updated_case.get('rejected_at') or datetime.now().isoformat()
        })
