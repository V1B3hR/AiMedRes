    ...
    def _update_safety_state(self):
        ...
        with self._lock:
            ...
            if self.safety_state != previous_state:
                logger.warning(
                    f"Safety state changed: {previous_state.value} → {self.safety_state.value}"
                )
    ...
    def _check_circuit_breaker(self):
        ...
        with self._lock:
            ...
            if (
                ...
                and now - self.circuit_breaker_last_failure > self.circuit_breaker_timeout
            ):
    ...
    def record_operation(
        ...
        with self._lock:
            ...
            if not success:
                ...
                if self.circuit_breaker_failures >= 5 and not self.circuit_breaker_open:
    ...
    def is_safe_to_operate(self) -> bool:
        ...
        with self._lock:
            ...
            if self.cpu_history:
                ...
                if len(recent_cpu) >= 3 and all(
                    cpu > self.max_cpu_percent * 1.5 for cpu in recent_cpu
                ):
                    return False
