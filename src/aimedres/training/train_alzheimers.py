...
class AlzheimerTrainingPipeline:
    ...
    def preprocess_data(self, target_column: str = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if target_column is None:
            ...
            if target_column is None:
                ...
                logger.error(
                    f"Could not auto-detect target column. Available columns: {available_cols}"
                )
                logger.error(
                    "Please specify the target column explicitly using the target_column parameter."
                )
                raise ValueError(
                    "Target column not found. Common target column names not detected in dataset"
                )
        ...
        logger.info(
            f"Class mapping: {dict(zip(self.label_encoder.classes_, self.label_encoder.transform(self.label_encoder.classes_)))}"
        )
    ...
    def train_classical_models(self, n_folds: int = 5) -> Dict[str, Dict[str, float]]:
        ...
        for model_name, model in models.items():
            ...
            try:
                ...
                # Save model
                model_path = (
                    self.output_dir / "models" / f"{model_name.lower().replace(' ', '_')}.pkl"
                )
    ...
    def _generate_summary_report(self, report: Dict[str, Any]):
        ...
        with open(summary_path, "w") as f:
            ...
            if report["dataset_info"]["shape"]:
                ...
                f.write(
                    f"  Features after preprocessing: {report['preprocessing_info']['num_features']}\n\n"
                )
    ...
    def run_full_pipeline(
        self, data_path: str = None, target_column: str = None, epochs: int = 100, n_folds: int = 5
    ) -> Dict[str, Any]:
        """
