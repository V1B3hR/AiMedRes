...
class HumanInLoopGatekeeper:
    ...
    def submit_recommendation(self, recommendation: ClinicalRecommendation) -> Dict[str, Any]:
        """
        ...
        self._create_audit_entry(
            event_type="recommendation_submitted", recommendation=recommendation, approval=None
        )
        ...
        if self.requires_approval(recommendation.risk_level):
            ...
            logger.info(
                f"Low risk recommendation {recommendation.recommendation_id} " f"auto-approved"
            )
            return {
                "status": "approved",
                "recommendation_id": recommendation.recommendation_id,
    ...
    def approve_recommendation(
        ...
        if not rationale or len(rationale.strip()) < 10:
            raise ValueError("Approval rationale is required and must be at least 10 characters")

        if recommendation_id not in self.pending_approvals:
            raise ValueError(f"Recommendation {recommendation_id} not found in pending approvals")
        ...
        self._create_audit_entry(
            event_type="recommendation_approved", recommendation=recommendation, approval=approval
        )
    ...
    def reject_recommendation(
        ...
        """
        if not rationale or len(rationale.strip()) < 10:
            raise ValueError("Rejection rationale is required and must be at least 10 characters")
        ...
        self._create_audit_entry(
            event_type="recommendation_rejected", recommendation=recommendation, approval=approval
        )
    ...
    def admin_override(
        self, recommendation_id: str, admin_id: str, override_reason: str
    ) -> Dict[str, Any]:
        """
        ...
        if not override_reason or len(override_reason.strip()) < 20:
            raise ValueError("Override reason is required and must be at least 20 characters")
        ...
        logger.warning(
            f"ADMIN OVERRIDE: Recommendation {recommendation_id} " f"overridden by admin {admin_id}"
        )
    ...
    def get_audit_history(
        self, recommendation_id: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        if recommendation_id:
            entries = [
                e for e in entries if e["recommendation"]["recommendation_id"] == recommendation_id
            ]
