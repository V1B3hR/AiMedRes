...
PHI_CATEGORY_PATTERNS: Dict[str, Tuple[re.Pattern, str]] = {
    ...
    "address": (
        re.compile(
            r"\b\d{1,5}\s+[A-Z][a-zA-Z]+\s+(Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Blvd|Boulevard)\b"
        ),
        "[ADDRESS]",
    ),
...
def sanitize_phi(
    text: str, categories: Optional[Set[str]] = None, whitelist: Optional[Set[str]] = None
) -> str:
    """
...
def filter_adversarial_terms(
    text: str, embed_mgr: EmbeddingManager, cfg: AdversarialFilterConfig
) -> str:
    ...
    for tok in tokens:
        ...
        if df < cfg.min_doc_freq and (
            len(tok) >= cfg.long_token_length
            or ADVERSARIAL_TOKEN_PATTERN.match(tok)
            or sum(c.isdigit() for c in tok) >= 3
...
class MemoryStore:
    ...
    def store_memory(
        ...
        if self.sanitize:
            content = sanitize_phi(
                content, categories=self.phi_categories, whitelist=self.domain_whitelist
            )
    ...
    def retrieve(
        ...
        if sanitize_query and self.sanitize:
            query = sanitize_phi(
                query, categories=self.phi_categories, whitelist=self.domain_whitelist
            )
    ...
    def save_state(self, path: str):
        data = {
            ...
            "memories": {
                sid: [
                    m.to_json() if self.persist_vectors else {**m.to_json(), "vector": []}
                    for m in arr
                ]
...
RECOMMENDATIONS: Dict[str, Dict[str, List[str]]] = {
    ...
    "en": {
        "risk_stratification": [
            "Aggregate genetic & cognitive indicators",
            "Document explicit tier rationale",
        ],
        "prognostic": ["Schedule periodic reassessment", "Track functional & cognitive slope"],
        "therapeutic_planning": [
            "Optimize modifiable lifestyle factors",
            "Define reassessment triggers",
        ],
        "uncertainty_quantification": [
            "Acquire more longitudinal data",
            "Validate with external sources",
        ],
        "causal_analysis": [
            "Map hypothesized mediator pathways",
    ...
    "es": {
        "risk_stratification": [
            "Integrar indicadores genéticos y cognitivos",
            "Documentar la justificación del nivel",
        ],
        "prognostic": [
            "Programar reevaluaciones periódicas",
            "Seguir la pendiente funcional y cognitiva",
        ],
        "therapeutic_planning": [
            "Optimizar factores modificables",
            "Definir disparadores de reevaluación",
        ],
        "uncertainty_quantification": [
            "Obtener más datos longitudinales",
            "Validar con fuentes externas",
        ],
        "causal_analysis": [
            "Mapear vías mediadoras hipotéticas",
    ...
    "fr": {
        "risk_stratification": [
            "Intégrer indicateurs génétiques et cognitifs",
            "Documenter la justification du palier",
        ],
        "prognostic": [
            "Planifier réévaluations périodiques",
            "Suivre pente fonctionnelle et cognitive",
        ],
        "therapeutic_planning": [
            "Optimiser facteurs modifiables",
            "Définir déclencheurs de réévaluation",
        ],
        "uncertainty_quantification": [
            "Acquérir davantage de données longitudinales",
            "Valider par sources externes",
        ],
        "causal_analysis": [
            "Cartographier voies médiatrices hypothétiques",
            "Distinguer corrélation vs causalité",
        ],
        "general": [
            "Recueillir données structurées supplémentaires",
            "Maintenir un large différentiel",
        ],
    },
    "de": {
        "risk_stratification": [
            "Genetische & kognitive Indikatoren bündeln",
            "Begründung der Risikostufe dokumentieren",
        ],
        "prognostic": [
            "Regelmäßige Reevaluation planen",
            "Funktionellen & kognitiven Verlauf verfolgen",
        ],
        "therapeutic_planning": [
            "Modifizierbare Faktoren optimieren",
            "Neubewertungs-Trigger definieren",
        ],
        "uncertainty_quantification": [
            "Mehr Longitudinaldaten sammeln",
            "Mit externen Quellen validieren",
        ],
        "causal_analysis": [
            "Hypothetische Mediatorpfade abbilden",
            "Korrelation von Kausalität trennen",
        ],
        "general": [
            "Zusätzliche strukturierte Anamnese sammeln",
            "Breiten Differenzialraum erhalten",
        ],
    },
    "pt": {
        "risk_stratification": [
            "Integrar indicadores genéticos e cognitivos",
            "Documentar a justificativa do nível",
        ],
        "prognostic": [
            "Agendar reavaliações periódicas",
            "Monitorar declínio funcional e cognitivo",
        ],
        "therapeutic_planning": [
            "Otimizar fatores modificáveis",
            "Definir gatilhos de reavaliação",
        ],
        "uncertainty_quantification": [
            "Coletar mais dados longitudinais",
            "Validar com fontes externas",
        ],
        "causal_analysis": [
            "Mapear vias mediadoras hipotéticas",
    ...
    "it": {
        "risk_stratification": [
            "Integrare indicatori genetici e cognitivi",
            "Documentare la motivazione del livello",
        ],
        "prognostic": [
            "Pianificare rivalutazioni periodiche",
            "Monitorare declino funzionale e cognitivo",
        ],
        "therapeutic_planning": [
            "Ottimizzare fattori modificabili",
            "Definire trigger di rivalutazione",
        ],
        "uncertainty_quantification": [
            "Raccogliere più dati longitudinali",
            "Validare con fonti esterne",
        ],
        "causal_analysis": [
            "Mappare vie mediatrici ipotetiche",
            "Distinguere correlazione da causalità",
        ],
        "general": [
            "Raccogliere anamnesi strutturata aggiuntiva",
            "Mantenere ampio spettro differenziale",
        ],
    },
    "zh": {
...
def local_rt_desc(rt: str, lang: str) -> str:
    return RT_DESCRIPTIONS.get(lang, RT_DESCRIPTIONS["en"]).get(
        rt, RT_DESCRIPTIONS["en"]["general"]
    )
...
class TemplateGenerator:
    def generate(
        ...
        scores = (
            ", ".join([f"{s['kind']}={s['value']}" for s in features.get("scores", [])]) or "none"
        )
        ...
        lines = (
            ...
            + [f"- {r}" for r in recs]
            + [cot_section, f"Confidence≈{confidence:.2f}", disclaimer]
        )
        out = "\n".join([ln for ln in lines if ln.strip()])
        return re.sub(r"\n{3,}", "\n\n", out).strip()

    def _recommendations(
        self, lang: str, reasoning_type: str, features: Dict[str, Any]
    ) -> List[str]:
...
class PureLiteReasoningAgent:
    def __init__(
        ...
        logger.info(
            f"PureLiteReasoningAgent initialized: session={self.session_id} dim={self.store.embedding_manager.config.dim} weighting={self.store.embedding_manager.config.weighting}"
        )
    ...
    def reason(
        self, query: str, reasoning_type: str = "general", language: Optional[str] = None
    ) -> ReasoningResult:
        ...
        if self.sanitize_query and self.store.sanitize:
            query = sanitize_phi(
                query, categories=self.store.phi_categories, whitelist=self.store.domain_whitelist
            )

        raw = self.store.retrieve(
            self.session_id, query, limit=self.max_context_memories, sanitize_query=False
        )
    ...
    def _skeleton(
        self, reasoning_type: str, memories: List[MemoryRecord], features: Dict[str, Any]
    ) -> str:
        count = len(memories)
        ...
        if reasoning_type == "deductive":
            premises = sum(
                1
                for m in memories
                if any(k in m.content.lower() for k in ["if ", " therefore", " implies", " when "])
            )
        ...
        if reasoning_type == "therapeutic_planning":
            return (
                "Therapeutic skeleton: optimize modifiable domains → define reassessment triggers."
            )
...
def demo():
    ...
    agent = PureLiteReasoningAgent(
        "pure_lite_agent", store, similarity_threshold=0.5, max_context_memories=7, enable_cot=True
    )

    seed_memories = [
        (
            "MMSE scores below 24 may indicate cognitive impairment requiring further evaluation.",
            "knowledge",
            0.9,
        ),
        (
            "Patient John Smith had APOE4 genotype reported on 2024-08-11 with mild memory complaints.",
            "knowledge",
            0.88,
        ),
        (
            "Lifestyle interventions including aerobic exercise can modulate cognitive decline trajectory.",
            "knowledge",
            0.83,
        ),
        (
            "CSF biomarkers (amyloid, tau) refine prodromal differential considerations.",
            "knowledge",
            0.8,
        ),
        (
            "Longitudinal change over 12 months may reveal accelerated decline patterns.",
            "knowledge",
            0.78,
        ),
        (
            "Comprehensive assessment should include cognitive testing, biomarker analysis, and family history.",
            "reasoning",
            0.76,
        ),
    ...
    queries = [
        (
            "What should be considered for MMSE score 22 and APOE4 positive?",
