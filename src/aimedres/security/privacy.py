...
class PrivacyManager:
    ...
    def register_data_for_retention(self, data_id: str, data_type: str) -> bool:
        ...
        try:
            ...
            security_logger.info(
                f"Registered {data_type} data {data_id} for retention until {retention_until}"
            )
            return True
        except Exception as e:
    ...
    def perform_automatic_cleanup(self) -> Dict[str, int]:
        ...
        """
        stats = {"anonymized_count": 0, "deleted_count": 0, "errors": 0}
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                anonymize_cutoff = datetime.now() - timedelta(
                    days=self.retention_policy.anonymize_after_days
                )
                cursor.execute(
                    """
