...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--dataset-choice", choices=["als-progression", "bram-als"], default="als-progression"
    )
...
class ALSTrainingPipeline:
        ...
        if KAGGLEHUB_AVAILABLE:
            try:
                ...
                if self.cfg.dataset_choice == "als-progression":
                    dpath = kagglehub.dataset_download(
        ...
        try:
            ...
            if task == "regression":
                ...
                param_dist = {
                    ...
                    "LogisticRegression": {"C": [0.1, 0.5, 1.0, 2.0]},
        ...
        for name, model in models.items():
            ...
            try:
                cv_res = cross_validate(
                    model, self.X_processed, self.y_processed, cv=cv, scoring=scoring
                )
    ...
    def train_classification(self) -> Dict[str, Dict[str, float]]:
        ...
        cv = StratifiedKFold(
            n_splits=self.cfg.folds, shuffle=True, random_state=self.cfg.random_seed
        )
        ...
        for name, model in models.items():
            ...
            try:
                ...
                logger.info(
                    f"{name} -> Acc={results[name]['accuracy_mean']:.4f} F1={results[name]['f1_macro_mean']:.4f}"
                )
            except Exception as e:
    ...
    def train_neural(self) -> Dict[str, float]:
        ...
        train_loader = DataLoader(
            TensorDataset(X_train, y_train), batch_size=self.cfg.batch_size, shuffle=True
        )
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, patience=self.cfg.lr_reduction_patience, factor=self.cfg.lr_reduction_factor
        )
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            if epoch == 1 or epoch % 10 == 0:
                ...
                logger.info(
                    f"[NN] Epoch {epoch}/{self.cfg.epochs} loss={running_loss/len(train_loader):.4f} "
    ...
    def _final_nn_eval(self, model, X_val, y_val, device):
        ...
        with torch.no_grad():
            ...
            if self.task_type == "classification":
                ...
                return {"best_validation_accuracy": acc, "validation_f1_macro": f1}
            else:
                outputs = outputs.squeeze()
                ...
                return {"best_validation_rmse": rmse, "validation_mae": mae, "validation_r2": r2}
    ...
    def compute_feature_importance(self):
        ...
        for base in ["RandomForest", "XGBoost", "GradientBoosting"]:
            fname = (
                f"{'regression' if self.task_type=='regression' else 'classification'}_{base}.pkl"
            )
        ...
        for name, model in candidates:
            if hasattr(model, "feature_importances_"):
                try:
                    ...
                    df_imp = pd.DataFrame(
                        {"feature": self.encoded_feature_names, "importance": fi}
        ...
        if self.cfg.compute_shap and SHAP_AVAILABLE and candidates:
            ...
            try:
                if "xgb" in str(type(model)).lower() or hasattr(model, "feature_importances_"):
                    ...
                    shap.summary_plot(
                        shap_values, sample, feature_names=self.encoded_feature_names, show=False
                    )
                    if MPL_AVAILABLE:
                        plt.tight_layout()
    ...
    def export_metrics(self):
        report = {
            ...
            "feature_count": (
                len(self.encoded_feature_names) if self.encoded_feature_names else None
            ),
        }
        ...
        if rows:
            pd.DataFrame(rows).to_csv(
                self.output_dir / "metrics" / "model_metrics.csv", index=False
            )
        ...
        with open(self.output_dir / "metrics" / "summary.txt", "w") as f:
            ...
            if self.regression_results:
                ...
                f.write(
                    f"Best Regression: {best_r[0]} RMSE={best_r[1]['rmse_mean']:.4f} R2={best_r[1]['r2_mean']:.4f}\n"
                )
            if self.classification_results:
                best_c = max(
                    self.classification_results.items(), key=lambda x: x[1]["accuracy_mean"]
                )
                f.write(
                    f"Best Classification: {best_c[0]} Acc={best_c[1]['accuracy_mean']:.4f} "
                    f"F1={best_c[1]['f1_macro_mean']:.4f}\n"
                )
            if self.neural_results:
                if self.task_type == "classification":
                    f.write(
                        f"Neural Best Acc: {self.neural_results.get('best_validation_accuracy')}\n"
                    )
                else:
                    f.write(
                        f"Neural Best RMSE: {self.neural_results.get('best_validation_rmse')}\n"
                    )
...
def main():
    ...
    try:
        ...
        if results["classification_results"]:
            best_c = max(
                results["classification_results"].items(), key=lambda x: x[1]["accuracy_mean"]
            )
            print(
                f"Best Classification: {best_c[0]} Acc={best_c[1]['accuracy_mean']:.4f} "
                f"F1={best_c[1]['f1_macro_mean']:.4f}"
