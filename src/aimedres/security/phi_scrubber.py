...
class PHIScrubber:
    ...
    PHI_PATTERNS = {
        ...
        "address": re.compile(
            r"\b\d{1,5}\s+[A-Z][a-zA-Z\s]+(?:Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Boulevard|Blvd|Drive|Dr|Court|Ct|Way)\b",
            re.IGNORECASE,
        ),
        ...
        "fax": re.compile(
            r"\b(?:fax|FAX)[:\s]*(?:\+?\d{1,3}[\s\-\.]?)?(?:\(?\d{3}\)?[\s\-\.]?)?\d{3}[\s\-\.]?\d{4}\b",
            re.IGNORECASE,
        ),
        ...
        "mrn": re.compile(
            r"\b(?:MRN|Medical\s+Record\s+Number|Patient\s+ID)[:\s#]*\d{5,}\b", re.IGNORECASE
        ),
    ...
    def __init__(
        self, aggressive: bool = True, hash_identifiers: bool = True, preserve_years: bool = True
    ):
        """
