...
class DiabetesTrainingPipeline:
    ...
    def load_data(
        self, data_path: str = None, dataset_choice: str = "early-diabetes"
    ) -> pd.DataFrame:
        """
        ...
        if data_path and os.path.exists(data_path):
            ...
            try:
                if dataset_choice == "early-diabetes":
                    ...
                    logger.warning(
                        f"Unknown dataset choice: {dataset_choice}. Using early-diabetes."
                    )
                    path = kagglehub.dataset_download("andrewmvd/early-diabetes-classification")
    ...
    def _create_sample_data(self) -> pd.DataFrame:
        ...
        for i in range(n_samples):
            ...
            if data["Age"][i] > 50:
                prob += 0.3
            if data["Polyuria"][i] == "Yes":
                prob += 0.4
            if data["Polydipsia"][i] == "Yes":
                prob += 0.3
            if data["sudden_weight_loss"][i] == "Yes":
                prob += 0.2
            if data["Obesity"][i] == "Yes":
                prob += 0.2
    ...
    def train_classical_models(self, n_folds: int = 5) -> Dict[str, Dict[str, float]]:
        ...
        for name, model in models.items():
            ...
            try:
                ...
                logger.info(
                    f"{name} - Accuracy: {results[name]['accuracy']:.4f} (±{results[name]['accuracy_std']:.4f})"
                )
    ...
    def save_training_report(self, classical_results: Dict, nn_results: Dict):
        ...
        report = {
            ...
            "dataset_info": {
                ...
                "target_classes": (
                    self.target_encoder.classes_.tolist() if self.target_encoder else None
                ),
            },
        ...
        with open(txt_path, "w") as f:
            ...
            for model_name, metrics in classical_results.items():
                ...
                f.write(
                    f"  Balanced Accuracy: {metrics['balanced_accuracy']:.4f} (±{metrics['balanced_accuracy_std']:.4f})\n"
                )
    ...
    def run_full_pipeline(
        ...
        epochs: int = 20,
        n_folds: int = 5,
        dataset_choice: str = "early-diabetes",
    ) -> Dict[str, Any]:
        """
    ...
    try:
        ...
        if "classical_models" in report:
            ...
            for model_name, metrics in report["classical_models"].items():
                print(
                    f"  {model_name}: Accuracy={metrics['accuracy']:.2%}, F1={metrics['f1_macro']:.2%}"
                )
