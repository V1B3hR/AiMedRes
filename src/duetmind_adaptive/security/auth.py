...
class SecureAuthManager:
    ...
    def validate_api_key(self, provided_key: Optional[str]) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """
        ...
        if provided_key in self.api_keys:
            ...
            security_logger.info(f"Successful auth for user: {key_info['user_id']} from {client_ip}")
    ...
    def _is_locked_out(self, client_ip: str) -> bool:
        ...
        if attempts_info['count'] >= self.max_failed_attempts:
            ...
            if datetime.now() - attempts_info['last_attempt'] > timedelta(minutes=self.lockout_duration):
                del self.failed_attempts[client_ip]
                return False
            return True
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if required_role and not auth_manager.has_role(user_info, required_role):
                return jsonify({'error': f'Insufficient permissions. Role {required_role} required'}), 403
