...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("parkinsons_training.log"), logging.StreamHandler()],
)
...
class ParkinsonsTrainingPipeline:
    ...
    def __init__(self, output_dir: str = "parkinsons_outputs"):
        ...
        logger.info(
            f"Initialized Parkinson's Training Pipeline. Outputs will be saved to {self.output_dir}"
        )
        ...
        if data_path and os.path.exists(data_path):
            ...
            try:
                if dataset_choice == "vikasukani":
                    ...
                    data_files = list(Path(dataset_path).glob("*.csv")) + list(
                        Path(dataset_path).glob("*.data")
                    )
                    if data_files:
                    ...
                    dataset_path = kagglehub.dataset_download(
                        "anonymous6623/uci-parkinsons-datasets"
                    )
                    data_files = list(Path(dataset_path).glob("*.csv")) + list(
                        Path(dataset_path).glob("*.data")
                    )
    ...
    def _load_from_directory(self, dir_path: str) -> pd.DataFrame:
        ...
        if pet_sbr_file:
            ...
            sbr_columns = [
                ...
                if col.startswith("PET_")
                and any(region in col for region in ["CAUD", "PUT", "CBM", "OCCIP"])
            ]
            ...
            if caudate_cols and putamen_cols:
                ...
                logger.info(
                    f"Created target variable 'status' based on median SBR split (median={median_sbr:.3f})"
                )
    ...
    def _create_sample_parkinsons_data(self) -> pd.DataFrame:
        ...
        data = {
            ...
            "MDVP:Fhi(Hz)": np.random.normal(
                200, 40, n_samples
            ),  # Maximum vocal fundamental frequency
            "MDVP:Flo(Hz)": np.random.normal(
                120, 25, n_samples
    ...
    def preprocess_data(self, target_column: str = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if y.dtype == "object":
            ...
            logger.info(
                f"Encoded target labels: {dict(zip(self.label_encoder.classes_, range(len(self.label_encoder.classes_))))}"
            )
        else:
        ...
        logger.info(
            f"Found {len(categorical_features)} categorical and {len(numerical_features)} numerical features"
        )
    ...
    def train_classical_models(self, n_folds: int = 5) -> Dict[str, Dict[str, float]]:
        ...
        for name, model in models.items():
            ...
            try:
                ...
                # Save model
                model_path = (
                    self.output_dir / "models" / f"{name.replace(' ', '_').lower()}_model.pkl"
                )
                with open(model_path, "wb") as f:
                    pickle.dump(model, f)

                logger.info(
                    f"{name} - Accuracy: {results[name]['accuracy_mean']:.3f} (±{results[name]['accuracy_std']:.3f})"
                )
    ...
    def train_neural_network(self, epochs: int = 100, batch_size: int = 32) -> Dict[str, float]:
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc:
                ...
                # Save best model
                torch.save(
                    model.state_dict(), self.output_dir / "models" / "neural_network_best.pth"
                )
            else:
                patience_counter += 1
            ...
            if (epoch + 1) % 20 == 0:
                logger.info(
                    f"Epoch [{epoch+1}/{epochs}], Loss: {avg_train_loss:.4f}, Val Acc: {val_acc:.4f}"
                )
        ...
        with open(summary_path, "w") as f:
            ...
            if report["classical_results"]:
                ...
                for model, metrics in report["classical_results"].items():
                    ...
                    f.write(
                        f"  Balanced Accuracy: {metrics['balanced_accuracy_mean']:.3f} (±{metrics['balanced_accuracy_std']:.3f})\n"
                    )
                    f.write(
                        f"  F1 Score: {metrics['f1_macro_mean']:.3f} (±{metrics['f1_macro_std']:.3f})\n"
                    )
                    f.write(
            ...
            if report["neural_results"]:
                ...
                f.write("-" * 22 + "\n")
                f.write(
                    f"Best Validation Accuracy: {report['neural_results']['best_validation_accuracy']:.3f}\n"
                )
                f.write(
                    f"Final Validation F1: {report['neural_results']['validation_f1_macro']:.3f}\n"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Parkinson's Disease Classification Training Pipeline"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    parser.add_argument(
        "--epochs", type=int, default=100, help="Number of epochs for neural network training"
    )
    ...
    parser.add_argument(
        "--output-dir", type=str, default="parkinsons_outputs", help="Output directory for results"
    )
    ...
    try:
        ...
        if results.get("classical_results"):
            ...
            best_model = max(
                results["classical_results"].items(), key=lambda x: x[1]["accuracy_mean"]
            )
            print(f"  {best_model[0]}: {best_model[1]['accuracy_mean']:.3f} accuracy")

        if results.get("neural_results"):
            print(
                f"Neural Network: {results['neural_results']['best_validation_accuracy']:.3f} best validation accuracy"
            )
